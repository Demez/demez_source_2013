...
//#include "hl2/c_demez_hl2_player.h"
...
static ConVar r_flashlightquadratic( "r_flashlightquadratic", "10000.0", FCVAR_CHEAT );
...
//-----------------------------------------------------------------------------
void CFlashlightEffect::UpdateLight(const Vector &vecPos, const Vector &vecForward, const Vector &vecRight, const Vector &vecUp )
{
	...
	const float flDistCutoff = 0.0f; // 128.0f
	...
	if ( pPlayer )
	{
		...
		if ( flBatteryPower <= 10.0f )
		{
			...
			if ( flScale < 0.35f )
			{
				...
				{
					// On
					state.m_fQuadraticAtten = r_flashlightlinear.GetFloat() * flScale;
				}
				else
				{
					// Off
			...
			else
			{
				...
				state.m_fQuadraticAtten = r_flashlightlinear.GetFloat() * flScale + 1.5f * flNoise;
			}
