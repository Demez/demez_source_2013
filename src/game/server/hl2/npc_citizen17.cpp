...
const int MAX_PLAYER_SQUAD = 400; // 4

ConVar	sk_citizen_health				( "sk_citizen_health",					"0");
ConVar	sk_citizen_squad_health			( "sk_citizen_squad_health",			"0");
				...
				if ( candidates[i].iSquadIndex == -1 )
				{
					for ( int j = 0; j < g_AI_Manager.NumAIs(); j++ )
					{
						...
						if ( ppAIs[j]->HasSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE ) )
							// bad idea?
							// change his health to a higher value so this guy isn't useless and dies in 2 seconds
							ppAIs[j]->m_iHealth = sk_citizen_squad_health.GetInt(); // um
							continue; 
...
bool CNPC_Citizen::ShouldLookForHealthItem()
{
	...
	if( pNearest && pNearest->GetHealth() <= pNearest->GetHealth() * 0.75f )
		return false;
